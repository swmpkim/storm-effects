...
*  Maximum 15-minute-averaged wind speed was __`r round(max(dat_storm$wspd_mph), 1)` mph__  
*  Maximum wind gust was __`r round(max(dat_storm$maxwspd_mph), 1)` mph__  
*  Minimum barometric pressure was __`r round(min(dat_storm$bp), 1)` mb (`r round(0.0295301 * min(dat_storm$bp), 2)` in)__
*  Maximum depth at the Bayou Heron water quality station (High tides in September 2017 were around 5 ft (4.8-5.6); have not calculated August 2018) was __`r round(max(dat_bh$depth_ft, na.rm=TRUE),1)` ft__  
*  Amount of precipitation received on September 4-5 was __`r round(sum(dat_storm$totprcp_in), 1)` in__
